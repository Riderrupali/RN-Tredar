  ...
  class _ChatScreenState extends State<ChatScreen> {
    ...
    void initState() {
      ...
      if (widget.initialMessage != null) {
        Future.delayed(const Duration(milliseconds: 300), () => _processInput(widget.initialMessage!));
      }
    }

    @override
    void dispose() { _ctrl.dispose(); _scroll.dispose(); _topicCtrl.dispose(); super.dispose(); }
    ...
    Future<void> _searchTopic(String query) async {
      if (query.trim().isEmpty) { setState(() => _topicResults = []); return; }
    ...
    void _insertTopicResult(KnowledgeItem item) {
      ...
      setState(() {
        ...
        _showTopicSearch = false; _topicResults = []; _topicCtrl.clear();
      });
      _scrollToBottom();
    ...
    Future<void> _processInput(String text) async {
      ...
      if (cmdResult != null) {
        if (cmdResult == '__SCREEN_MONITOR__') {
          ...
          if (mounted) Navigator.push(context, MaterialPageRoute(builder: (_) => const ScreenMonitorScreen()));
          return;
        }
        response = cmdResult;
      } else if (_awaitingSave) {
        await _ai.saveNewKnowledge(_pendingSaveTopic, text, null);
        response = _lang == 'marathi' ? '✅ "$_pendingSaveTopic" save झालं!' : '✅ Saved "$_pendingSaveTopic"!';
        _awaitingSave = false; _pendingSaveTopic = '';
      } else {
        ...
        if (response == '__MUTE__') {
          ...
          _awaitingSave = true; _pendingSaveTopic = text;
        }
      }
    ...
    Future<void> _uploadFile() async {
      ...
      try {
        ...
        _replaceLastAiMsg(response); _tts.speak(response);
      } catch (e) {
        _replaceLastAiMsg(_lang == 'marathi' ? '⚠️ File error. .txt वापरा.' : 'File error. Use .txt');
      }
    }
    ...
    Future<void> _scanImage() async {
      ...
      _addAiMsg(_lang == 'marathi' ? '📷 Image scan होतेय...' : '📷 Scanning...');
      try {
    ...
    void _addAiMsg(String text) {
      setState(() => _messages.add(ChatMessage(text: text, isUser: false, createdAt: DateTime.now())));
      _scrollToBottom();
    }
    ...
    void _scrollToBottom() {
      Future.delayed(const Duration(milliseconds: 200), () {
        if (_scroll.hasClients) _scroll.animateTo(
          _scroll.position.maxScrollExtent, duration: const Duration(milliseconds: 300), curve: Curves.easeOut);
      });
    }
          ...
          actions: [
            IconButton(
              ...
              onPressed: () => setState(() {
                ...
                if (!_showTopicSearch) { _topicResults = []; _topicCtrl.clear(); }
              }),
            ),
          ],
        ),
                ...
                return const Padding(padding: EdgeInsets.all(12),
                  child: Row(children: [
                    ...
                    SizedBox(width: 20, height: 20,
                      child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF9B59F5))),
                    SizedBox(width: 10),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          TextField(
            ...
            decoration: InputDecoration(
              hintText: '🧠 Topic search... (Support, RSI, Candle...)',
          ...
          if (_topicResults.isNotEmpty) SizedBox(
            height: 160,
            child: ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                return ListTile(
                  ...
                  title: Text(item.topic, style: const TextStyle(color: Colors.white, fontSize: 13)),
                  subtitle: Text(
    ...
    Widget _buildShortcuts() {
      const chips = [('123 🚀', '123'), ('13 6 📊', '13 6'), ('25 2 📉', '25 2'), ('3 2 1 ⛔', '3 2 1'), ('2 💾', '2')];
    ...
    Widget _buildInputBar() {
      return Container(
        ...
        color: const Color(0xFF1A1A2E),
        child: Row(children: [
          IconButton(icon: const Icon(Icons.attach_file, color: Colors.white38), onPressed: _uploadFile),
          IconButton(icon: const Icon(Icons.image_search, color: Colors.white38), onPressed: _scanImage),
          Expanded(child: TextField(
            ...
            decoration: InputDecoration(
              hintText: _lang == 'marathi' ? 'लिहा... (123, 13 6, 25 2, 3 2 1)' : 'Type or command (13 6, 123...)',
              hintStyle: const TextStyle(color: Colors.white38, fontSize: 13),
              filled: true, fillColor: const Color(0xFF0D0D1A),
