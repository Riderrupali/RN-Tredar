...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget _languageSelector() {
    return Row(
      ...
      final label = lang == AppLanguage.marathi
          ? '🇮🇳 Marathi'
  ...
  Widget _voiceSelector() {
    return Column(children: [
      _voiceRow('Female 🔊', VoiceType.female),
      const SizedBox(height: 8),
      _voiceRow('Male 🔊', VoiceType.male),
      const SizedBox(height: 8),
      _voiceRow('Child 🔊', VoiceType.child),
    ]);
  }

  Widget _voiceRow(String label, VoiceType type) {
    final selected = _tts.voiceType == type;
    return GestureDetector(
      onTap: () async {
        await _tts.setVoiceType(type);
        setState(() {});
      },
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected
              ? const Color(0xFF6C3CE1).withOpacity(0.2)
        ...
        child: Row(children: [
          Icon(selected ? Icons.radio_button_checked : Icons.radio_button_unchecked,
              color: selected ? const Color(0xFF9B59F5) : Colors.white38,
              size: 20),
  ...
  Widget _muteRow() {
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(
            child: Text(_tts.isMuted ? 'Muted' : 'Voice Active',
